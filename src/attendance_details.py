...
class Attendance:
    def add(self, values: list):
        for data in values:
            dc.mycursor.execute(
                "insert into records (student_id,attendance) values(%(id)s,%(attendance)s)",
                data,
            )
        dc.mydb.commit()
    ...
    def edit(self, val: dict, data: int):
        val["id"] = data
        quer = "UPDATE records set attendance=%(attendance)s where student_id =%(id)s"
    ...
    def show(self, id: int):
        dc.mycursor.execute(
            "select records.student_id,student_details.student_name,records.attendance FROM records join student_details ON records.student_id=student_details.id where student_details.id=%s",
            (id,),
        )
